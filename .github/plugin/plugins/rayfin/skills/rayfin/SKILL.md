...
- `rayfin dev` requires `RAYFIN_FEATURE_FLAGS=docker-local-dev` and Docker Desktop — confirm both are set before troubleshooting startup failures.
...
- `rayfin dev db apply` compiles TS decorators → DAB config → applies schema to the local database (requires `RAYFIN_FEATURE_FLAGS=docker-local-dev`).
...
- Apply storage config with `rayfin dev storage apply` (requires `RAYFIN_FEATURE_FLAGS=docker-local-dev`).
...
- Deployment metadata is recorded in `rayfin/.deployments.json` and merged into `rayfin/.env` (and `rayfin.yml`).
...
# Local dev (requires RAYFIN_FEATURE_FLAGS=docker-local-dev)
